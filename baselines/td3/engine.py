...
import logging
logger = logging.getLogger('robosuite.scripts.td3.engine')
...
class Engine(BaseEngine):
    ...
    def train(self, starting_timestep=1):
        num_episode = starting_timestep
        ...
        for i in count():
            ...
            if i < self.config['replay.init_size']:
                ...
                #action = [np.zeros(self.env.action_space.shape)] # TODO
            else:
                #action = [np.array([0, 0, 0])] # TODO
    ...
    def eval(self, n=None, **kwargs):
        logger.info("n: %d", n) # TODO - remove debug statement
        infos = []
